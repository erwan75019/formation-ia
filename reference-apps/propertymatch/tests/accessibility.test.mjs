...
test("toutes les pages principales exposent la cible du lien d’évitement", async () => {
  ...
  for (const path of ["app/page.tsx", "app/biens/page.tsx", "app/favoris/page.tsx", "components/PropertyDetail.tsx", "app/biens/[slug]/not-found.tsx"]) {
    assert.match(await read(path), /id="main-content"/, path);
  }
});

test("les favoris et le menu publient leurs états accessibles", async () => {
  const favorite = await read("components/FavoriteButton.tsx");
  const header = await read("components/Header.tsx");
