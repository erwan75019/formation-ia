...
export function buildFavoritePropertyUrl(slug: string) {
  return `/biens/${encodeURIComponent(slug)}?origine=favoris`;
}
...
export function buildPropertyReturnUrl(
  ...
  return parameters.origine === "favoris" ? "/favoris" : "/biens";
}
